...
from terrain_representation.utils.voxel_grid_utils import convert_batch_of_pts_to_vg
        ...
        for key, dl in dataloaders.items():
            ...
            for batch_idx, new_data_dict in enumerate(dl):
                ...
                for t, data_dict in enumerate(data_dicts):
                    ...
                    if args.use_extended_input and t > 0:
                        if args.use_prev_pred_as_input:
                            extension_vg = centroids.detach().clone()
                            extension_vg[~occupancy] = -1.0
                        else:
                            extension_vg = data_dicts[t - 1]["measured"]
                        merged_vg = torch.concat(
                            [extension_vg, data_dict["measured"]],
                            dim=-1,
                        )
                        measured = merged_vg
                    else:
                    ...
                    target_mesh = data_dict.get("mesh")
                    ...
                    losses = loss_func_dense(
                        data={
                            ...
                            "measured_vg": data_dict["measured"],
                            "is_synthetic": data_dict["is_synthetic"],
                        },
                        **common_loss_kwargs,
                    )
